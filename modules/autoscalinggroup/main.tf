...
resource "aws_launch_configuration" "launch_config" {
  ...
  user_data = templatefile("${path.module}/amazon_ecs_ami.yml", {
...
resource "aws_autoscaling_group" "this" {
  ...
  launch_configuration = aws_launch_configuration.launch_config[0].name
  ...
  tags = concat(list(map("key", "Name", "value", local.name, "propagate_at_launch", true)), local.tags_asg_format)
}
